...
function HamburgerMenuButton({ children }: HamburgerMenuButtonProps) {
  ...
  const utilGetMaxWidth = (element: Element | null) =>
    element?.clientWidth || 0;
  const utilGetScrollBarWidth = () =>
    window.innerWidth - document.documentElement.clientWidth;

  function OnRefLoad(
    callback: (button: HTMLLabelElement, content: HTMLElement) => void
  ) {
  ...
  const RepositionBurgerMenu = useCallback(function (
    ...
    button.style.transform = `translateX( calc(${
      utilGetMaxWidth(content.firstElementChild) - utilGetScrollBarWidth()
    }px - 100%) )`;
  },
  []);
  ...
  useEffect(() => {
    OnRefLoad((button, content) => {
      ...
      button.style.transitionDuration =
        getComputedStyle(content).animationDuration;
  ...
  useEffect(() => {
    const xGetter = () =>
      utilGetMaxWidth(contentRef?.current) || utilGetMaxWidth(document.body);
  ...
  const { containerBox, contentBox, switchBox, isDuplicated } = styles;
  ...
  return (
    <section className={containerBox}>
      ...
      <section className={contentBox} ref={contentRef}>
        <div>{children}</div>
        <div>{checked && <DarkModeSwitch className={switchBox} />}</div>
      </section>
    </section>
  );
}
...
function InitPointerIsOutSideHandle(
  xLimitGetter: () => number,
  callback: () => void
) {
